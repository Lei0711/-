...
class ChurnPredictionAgent:
    ...
    def process_query(self, query: str) -> DecisionResult:
        ...
        agent_result = self.agent.run_sync(query)
        
        # 从AgentRunResult中提取DecisionResult
        ...
        while True:
            try:
                ...
                if hasattr(result, 'output'):
                    print(f"output属性类型: {type(result.output)}")
                    print(f"output属性: {result.output}")
                    result = result.output
                elif hasattr(result, 'result'):
                    print(f"result属性类型: {type(result.result)}")
                    print(f"result属性: {result.result}")
